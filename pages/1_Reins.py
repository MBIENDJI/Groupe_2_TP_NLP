...
st.set_page_config(page_title="Détection Rénale", page_icon="🏥", layout="wide")
...
if st.button("🔄 Recommencer"):
    for key in ['kidney_disease_fr', 'kidney_confidence', 'kidney_response', 'kidney_history', 'kidney_summary', 'kidney_german']:
        if key in st.session_state:
            del st.session_state[key]
    st.rerun()

uploaded = st.file_uploader("📁 Charger une image CT scan du rein", type=["jpg", "jpeg", "png"])
    ...
    with col2:
        if st.button("🔬 Analyser l'image", type="primary"):
            ...
            if conf < CONFIDENCE_THRESHOLD:
                st.error(f"⚠️ Image non reconnue (confiance: {conf:.1%} < {CONFIDENCE_THRESHOLD:.0%})")
            else:
    ...
    if 'kidney_response' in st.session_state:
        ...
        if st.button("Générer résumé + traduction"):
            with st.spinner("Génération..."):
                st.session_state['kidney_summary'] = generate_summary(st.session_state['kidney_response'])
